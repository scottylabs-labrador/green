import { getDatabase, ref, set, push, remove, update, get, child, runTransaction } from "firebase/database";
...
export function writeGroceryItem(grocerylist: string | string[], name: string, member: string, quantity = 1) {
  const db = getDatabase();
...
export function updateGroceryItem(grocerylist: string, id: string, name: string, changeQuantity = 1, member: string) {
  const db = getDatabase();
...
export function removeGroceryItem(grocerylist, id) {
  const db = getDatabase();
  const itemRef = ref(db, "grocerylists/" +grocerylist+"/groceryitems/"); 
