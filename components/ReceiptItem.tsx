...
const ReceiptItem = ({ id, name, price, splits, colors = {}, matched, receiptId }: ReceiptItemProps) => {
  ...
  return (
    <View className={`flex-row items-center justify-center w-[85%] h-12 self-center px-2 border border-gray-300 rounded-lg ${matched ? 'bg-white' : 'bg-peach'}`}>
      <Text className="text-black flex-1">{name}</Text>

      <View className="flex-row items-center self-center space-x-2 max-w-[100px]">
       {splits ? (
          <FlatList
            data={Object.keys(splits || {})}
            renderItem={renderColor}
      ...
      <Text className="w-12 text-right mx-2">${price.toFixed(2)}</Text>
      <Link
        href={{
          pathname: '/unmatched/',
